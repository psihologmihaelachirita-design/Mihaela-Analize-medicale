        ...
        <div style={{ padding:'28px', overflowY:'auto' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'24px', position:'sticky', top:0, background:'#f8f9fa', zIndex:9, paddingTop:'12px', paddingBottom:'12px' }}>
            <div>
              ...
              <div style={{ fontSize:'12px', color:'#16705a', marginTop:'4px' }}>Datele de identitate (nume, CNP) se completează în <a href="/profil" style={{ color:'#16705a', fontWeight:700, fontSize:'15px', textDecoration:'underline' }}>Profil</a></div>
            </div>
              ...
              {!editMode ? (
                <>
                  ...
                  <button onClick={() => {
                    ...
                    if (w) {
                      w.document.write(`<html><head><style>body{font-family:system-ui;padding:24px;max-width:420px;margin:0 auto}h1{background:#16705a;color:white;padding:16px;border-radius:8px;text-align:center;font-size:18px;font-weight:500}.row{border:1px solid #e5e7eb;border-radius:8px;padding:12px;margin-bottom:8px}.lbl{font-size:11px;color:#888;text-transform:uppercase;margin-bottom:3px}.val{font-size:15px;font-weight:500}</style></head><body><h1>Card urgenta</h1><div class='row'><div class='lbl'>Grup sanguin</div><div class='val'>${grupSanguin || '-'}</div></div><div class='row'><div class='lbl'>Alergii medicamente</div><div class='val'>${alergiiMed.filter(Boolean).join(', ') || '-'}</div></div><div class='row'><div class='lbl'>Alergii alimentare</div><div class='val'>${alergiiAl.filter(Boolean).join(', ') || '-'}</div></div>${diagnostice.map(d => d.nume).filter(Boolean).length > 0 ? "<div class='row'><div class='lbl'>Diagnostice</div><div class='val'>" + diagnostice.map(d => d.nume).filter(Boolean).join(', ') + "</div></div>" : ''}${implanteList.map(i => i.nume).filter(Boolean).length > 0 ? "<div class='row'><div class='lbl'>Implanturi</div><div class='val'>" + implanteList.map(i => i.nume).filter(Boolean).join(', ') + "</div></div>" : ''}${interventii.map(i => i.nume).filter(Boolean).length > 0 ? "<div class='row'><div class='lbl'>Interventii</div><div class='val'>" + interventii.map(i => i.nume).filter(Boolean).join(', ') + "</div></div>" : ''}</body></html>`)
                      w.document.close()
                      w.print()
                    }
            ...
            <div style={{ background:'white', border:'0.5px solid #e5e7eb', borderRadius:'10px', padding:'14px', display:'flex', flexDirection:'column', gap:'8px', minHeight:'120px' }}>
              ...
              {editMode ? (
                <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                  ...
                  <Checkbox checked={alergiiMedAtestat} onChange={() => setAlergiiMedAtestat(!alergiiMedAtestat)} label="Document care atestă" />
                  <label style={{ display:'flex', alignItems:'center', gap:'6px', fontSize:'12px', color:'#16705a', cursor:'pointer', fontWeight:500 }}>
                    📎 Atașează document
                    <input type="file" accept=".pdf,image/*" style={{ display:'none' }} onChange={async e => {
                      const f = e.target.files?.[0]
              ...
              {alergiiMed.filter(a => a && a.trim() !== '').length > 0 && <div style={{ marginTop:'auto' }}><BadgeDoc atestat={alergiiMedAtestat} /></div>}
            </div>
            <div style={{ background:'white', border:'0.5px solid #e5e7eb', borderRadius:'10px', padding:'14px', display:'flex', flexDirection:'column', gap:'8px', minHeight:'120px' }}>
