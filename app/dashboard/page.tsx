    ...
    <div style={{fontFamily:'system-ui,-apple-system,sans-serif',background:'#f8f9fa',minHeight:'100vh'}}>
      ...
      <div style={{background:'white',borderBottom:'0.5px solid #e5e7eb',padding:'0 24px',height:'52px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:10}}>
        <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
          ...
          <span style={{fontSize:'14px',fontWeight:500,color:'#111'}}>MedFile</span>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
          <div style={{width:'30px',height:'30px',borderRadius:'50%',background:'#E1F5EE',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'10px',fontWeight:500,color:'#0F6E56'}}>{initiale}</div>
          <span style={{fontSize:'13px',color:'#888'}}>{username}</span>
          <Link href="/profil" style={{padding:'6px 14px',background:'transparent',border:'0.5px solid #e5e7eb',borderRadius:'8px',fontSize:'12px',color:'#555',textDecoration:'none'}}>Profil</Link>
          <button onClick={handleLogout} style={{padding:'6px 14px',background:'transparent',border:'0.5px solid #e5e7eb',borderRadius:'8px',fontSize:'12px',color:'#555',cursor:'pointer'}}>Ieșire</button>
        </div>
      </div>

      <div style={{maxWidth:'900px',margin:'0 auto',padding:'24px'}}>

        <h1 style={{fontSize:'18px',fontWeight:500,color:'#111',margin:'0 0 4px'}}>Dosarul meu medical</h1>
        ...
        <div style={{display:'flex',gap:'8px',marginBottom:'16px'}}>
          <Link href="/panoramic" style={{display:'flex',alignItems:'center',gap:'6px',padding:'8px 16px',background:'#1D9E75',color:'white',borderRadius:'8px',fontSize:'13px',fontWeight:500,textDecoration:'none'}}>
            📊 Vizualizare panoramică
          </Link>
          <Link href="/upload" style={{display:'flex',alignItems:'center',gap:'6px',padding:'8px 14px',background:'white',border:'0.5px solid #e5e7eb',borderRadius:'8px',fontSize:'13px',color:'#555',textDecoration:'none'}}>
            ↑ Adaugă analize
          </Link>
          <Link href="/export" style={{display:'flex',alignItems:'center',gap:'6px',padding:'8px 14px',background:'white',border:'0.5px solid #e5e7eb',borderRadius:'8px',fontSize:'13px',color:'#555',textDecoration:'none'}}>
            ↓ Export PDF
          </Link>
        </div>
        ...
        {analize.length === 0 ? (
          <div style={{background:'white',border:'0.5px solid #e5e7eb',borderRadius:'10px',padding:'3rem',textAlign:'center'}}>
            ...
            <p style={{fontSize:'14px',fontWeight:500,color:'#111',marginBottom:'8px'}}>Niciun buletin de analize încă</p>
            <p style={{fontSize:'13px',color:'#888',marginBottom:'16px'}}>Uploadează primul PDF și platforma va extrage automat toate valorile</p>
            <Link href="/upload" style={{padding:'10px 20px',background:'#1D9E75',color:'white',borderRadius:'8px',textDecoration:'none',fontSize:'13px'}}>Adaugă primul buletin</Link>
          </div>
        ) : (
            ...
            {analize.map((a,i) => {
              ...
              return (
                <div key={i} style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',padding:'10px 16px',borderBottom:i<analize.length-1?'0.5px solid #f0f0f0':'none',alignItems:'center'}}>
                  ...
                  <span style={{display:'inline-flex',padding:'3px 10px',background:badgeBg,color:badgeColor,borderRadius:'12px',fontSize:'11px',fontWeight:500,width:'fit-content'}}>{badgeLabel}</span>
                </div>
              )
            })}
          </div>
